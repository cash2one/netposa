...
exports.ErrorCode = {
    ...
    ERROR_NO_PARAM: -2,
    // ----------------行政区域相关 -10000+ -------------------------
    ERROR_REPEAT_AREA: -10001,
    // 行政区域父区域id为空
    ERROR_PARENT_AREA_ID_NULL: -10002,
    // 行政区域code唯一编码为空
    ERROR_AREA_CODE_NULL: -10003,
    // 行政区域id为空;
    ERROR_AREA_ID_NULL: -10004,
    // 行政区域不存在
    ERROR_NO_AREA: -10005,
    // 不能删除顶级行政区域
    ERROR_NOT_DELETE_ROOT_AREA: -10006,
    // 父级行政区域不存在
    ERROR_NO_PARENT_AREA: -10007,
    // 父行政区域不能为自身
    ERROR_PARENT_AREA_NOT_SELF: -10008,
    // 重复的灯杆编码
